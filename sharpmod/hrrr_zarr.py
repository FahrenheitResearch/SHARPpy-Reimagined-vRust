...
def fetch_hrrr_zarr_point(
    ...
    *,
    ...
    def missing_chunk(exc):
        """Zarr v2 represents an all-fill sparse chunk by omitting its key."""
        ...
        with ThreadPoolExecutor(max_workers=max(1, int(max_workers))) as executor:
            for level_index, level in enumerate(plan.levels):
                for field in plan.fields:
                    ...
                    future = executor.submit(load_chunk, url)
                    tasks[future] = (level_index, field, metadata)
            for future in as_completed(tasks):
                level_index, field, metadata = tasks[future]
                payload = future.result()
                # Missing chunk objects are valid sparse-Zarr fill chunks.
                # Use NaN rather than the on-disk -9999 sentinel so the shared
                # sounding column builder masks the below-ground level before
                # applying temperature/RH conversions.
                columns[field][level_index] = (
                    np.nan if payload is None else decode_zarr_point(
                        payload, metadata, iy=iy, ix=ix
                    )
                )
        dataset = _dataset_from_columns(
