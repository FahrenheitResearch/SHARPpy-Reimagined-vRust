...
class PickerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._model_cache_worker: _NativeHourCacheWorker | None = None
        self._model_busy_kind: str | None = None
    ...
    def _on_fetch_worker_finished(self) -> None:
        """Release a completed UWyo worker before enabling another fetch."""
        worker = self.sender()
    ...
    def _on_fetch_ok(self, npz_path, meta, when, prof_col=None,
                     stn_id=None) -> None:
        self.statusBar().showMessage(
        ...
        QApplication.processEvents()
        try:
            # Newer workers decode and warm the display profile off the UI
            # thread. Keep a compatibility fallback for the historical
            # three-value signal used by extensions and older frozen builds.
            if prof_col is None:
    ...
    def _build_model_tab(self) -> QWidget:
        ...
        self._model_cache_btn = QPushButton(
            "Cache This Hour for Fast Map Browsing")
        self._model_cache_btn.setMinimumHeight(34)
        self._model_cache_btn.setToolTip(
            "Download and decode the complete selected model hour once. "
            "Later map clicks at this run and forecast hour can then read "
            "nearby soundings directly from the local Rust .rws store.")
        self._model_cache_btn.clicked.connect(self._cache_model_hour)
        left.addWidget(self._model_cache_btn)
        ...
        left_w.setMinimumWidth(PICKER_RAIL_MIN_WIDTH)
        left_w.setMaximumWidth(PICKER_RAIL_MAX_WIDTH)
        # Availability, valid-time, and provider notes can exceed a 720 px
        # desktop. Preserve each group's natural height and scroll the rail
        # instead of letting Qt compress Forecast into the Point header.
        left_w.setMinimumHeight(max(760, left_w.sizeHint().height()))
    ...
    def _clear_model_viewer(self, viewer_id: int | None = None) -> None:
        """Forget the tracked forecast viewer when its Qt object is destroyed."""
        viewer = getattr(self, "_model_viewer", None)
        if viewer is None or viewer_id is None or id(viewer) == viewer_id:
            self._model_viewer = None
    ...
    def _model_viewer_is_open(self) -> bool:
        ...
        try:
            ...
            visible = False
        if not visible:
            self._clear_model_viewer(id(viewer))
        return bool(visible)

    def _track_model_viewer(self, viewer, menu_name: str) -> None:
        """Track the one viewer/profile replaced by forecast map clicks."""
        if viewer is not getattr(self, "_model_viewer", None):
            viewer_id = id(viewer)
            viewer.destroyed.connect(
                lambda *_args, viewer_id=viewer_id:
                self._clear_model_viewer(viewer_id))
        self._model_viewer = viewer
        self._model_viewer_menu = menu_name

    def _replace_model_profile(self, viewer, prof_col) -> str:
        """Replace only this picker's prior model profile in ``viewer``.

        Other observed, local, or session collections remain untouched. When
        SHARPpy generates the same menu key for a new forecast hour, remove the
        ...
        if old_menu and PickerWindow._replace_model_profile_in_place(
            ...
            if not PickerWindow._replace_same_menu_model_profile(
                ...
                # Test doubles and alternate SPCWindow implementations may not
                # expose the vendored collection lists. This fallback is safe
                # when at least one other profile keeps the viewer populated.
                viewer.rmProfileCollection(old_menu)
                viewer.addProfileCollection(
    ...
    def _replace_model_profile_in_place(
        ...
        for widget_name in ("sound", "hodo"):
            widget_collections = getattr(
                getattr(sw, widget_name, None), "prof_collections", None)
            if not isinstance(widget_collections, list):
                continue
            try:
                target_index = widget_collections.index(old_prof)
            except ValueError:
                continue
    ...
    def _model_is_busy(self) -> bool:
        """Whether a foreground forecast fetch or native hour build is active."""
        return self._model_worker is not None \
            or getattr(self, "_model_cache_worker", None) is not None

    def _model_cache_availability(self, cfg) -> tuple[bool, str]:
        """Return whether the selected hour can use the explicit Rust cache."""
        if cfg is None:
            return False, "Choose a forecast model first."
        try:
            from sharpmod.tools import rusty_weather
            if cfg.key not in rusty_weather.SUPPORTED_MODELS:
                return False, (
                    f"{cfg.label} is not supported by the Rust .rws store.")
            if rusty_weather.backend_mode() == "python":
                return False, (
                    "Rust model acquisition is disabled by "
                    "SHARPMOD_MODEL_BACKEND=python.")
            if rusty_weather.find_binaries() is None:
                return False, (
                    "Rusty Weather cache helpers are not installed.")
        except Exception as exc:  # noqa: BLE001 - keep picker usable
            return False, f"Rust model-hour caching is unavailable: {exc}"
        return True, (
    ...
    def _model_fetch(self) -> None:
        ...
        click_timer = getattr(self, "_model_click_timer", None)
        ...
        if self._model_worker is not None \
                or getattr(self, "_model_cache_worker", None) is not None:
            QMessageBox.information(self, APP_NAME,
                                    "A forecast-model operation is already in progress.")
            return
        lat = float(self._model_lat.value())
        lon = float(self._model_lon.value())
    ...
    def _cache_model_hour(self) -> None:
        """Build the selected full-hour ``.rws`` cache in the background."""
        click_timer = getattr(self, "_model_click_timer", None)
        if click_timer is not None:
            click_timer.stop()
        cfg = self._model_config()
        if cfg is None:
            QMessageBox.warning(self, APP_NAME, "Choose a forecast model first.")
            return
        if self._model_is_busy():
            QMessageBox.information(
                self, APP_NAME,
                "A forecast-model operation is already in progress.")
            return
        cache_available, detail = self._model_cache_availability(cfg)
        ...
        run_time = self._model_run_time()
        fxx = self._model_selected_fxx()
        worker = _NativeHourCacheWorker(
            cfg.key, run_time, fxx, parent=self)
        self._model_cache_worker = worker
        self._set_model_busy(True, operation="cache")
        self.statusBar().showMessage(
            f"Caching {cfg.label} {run_time:%Y-%m-%d %H}Z "
            f"F{fxx:03d} for fast map browsing\u2026")
        worker.finished_ok.connect(self._on_model_hour_cache_ok)
        worker.failed.connect(self._on_model_hour_cache_failed)
        worker.cancelled.connect(self._on_model_hour_cache_cancelled)
        worker.progress.connect(self._on_model_fetch_progress)
        worker.finished.connect(self._on_model_hour_cache_finished)
        _LOGGER.info(
            "native_hour_cache.start model=%s run=%s fxx=%03d",
            cfg.key, run_time, fxx)
        worker.start()
    ...
    def _shutdown_model_cache(self) -> None:
        ...
        if self._model_cache_worker is not None:
            self._model_cache_worker.requestInterruption()
            self._model_cache_worker.wait(5000)
    ...
    def _on_model_hour_cache_ok(
            self, model: str, run_time: datetime, fxx: int,
            hour_path: str) -> None:
        """Surface a durable native cache hit without opening a sounding."""
        try:
            from sharpmod.tools import model_extract
            label = model_extract.get_config(model).label
        except Exception:  # noqa: BLE001 - selection may change mid-download
            label = str(model).upper()
        self._model_cached_hour = (
            str(model), run_time, int(fxx), os.fspath(hour_path))
        message = (
            f"Cached {label} {run_time:%Y-%m-%d %H}Z F{int(fxx):03d}; "
            "map clicks at this hour can now use the fast local .rws store")
        _LOGGER.info(
            "native_hour_cache.ready model=%s run=%s fxx=%03d path=%s",
            model, run_time, int(fxx), hour_path)
        self.statusBar().showMessage(message, 10000)

    def _on_model_hour_cache_failed(self, message: str) -> None:
        _LOGGER.error("native_hour_cache.failed message=%s", message)
        self.statusBar().showMessage("Model-hour cache failed")
        QMessageBox.critical(self, APP_NAME, message)

    def _on_model_hour_cache_cancelled(self) -> None:
        _LOGGER.info("native_hour_cache.cancelled")
        self.statusBar().showMessage("Model-hour cache cancelled", 5000)

    def _on_model_hour_cache_finished(self) -> None:
        """Release the native cache worker and restore forecast controls."""
        worker = self.sender()
        if self._model_cache_worker is not worker:
            _LOGGER.warning(
                "native_hour_cache.finished_stale worker=%s current=%s",
                id(worker), id(self._model_cache_worker))
            worker.deleteLater()
            return
    ...
    def _set_model_busy(self, busy: bool, operation: str = "fetch") -> None:
        _LOGGER.debug(
            "model_fetch.ui_busy busy=%s operation=%s fetch_worker=%s "
            "cache_worker=%s",
            busy, operation,
            id(self._model_worker) if self._model_worker else None,
            id(self._model_cache_worker) if self._model_cache_worker else None)
        if busy:
            self._model_busy_kind = operation
            self._model_fetch_btn.setEnabled(False)
            self._model_cache_btn.setEnabled(False)
    ...
    def _on_model_fetch_progress(self, stage: str, total_bytes: int) -> None:
        ...
        if getattr(self, "_model_busy_kind", None) == "cache":
            messages = {
                "locating": (
                    "Checking the native model-hour cache\u2026", "Checking\u2026"),
                "cached": (
                    "Selected model hour is already cached\u2026",
                    "Already Cached\u2026"),
                "complete": (
                    "Finalizing the fast map-browsing cache\u2026",
                    "Finalizing\u2026"),
            }
        else:
            messages = {
                ...
                "decoding": (
                    "Decoding downloaded GRIB fields\u2026", "Decoding\u2026"),
                "cached": ("Using cached model hour\u2026", "Extracting\u2026"),
                "extracting": (
                    "Extracting the nearest grid point\u2026", "Extracting\u2026"),
                "writing": ("Writing the point sounding\u2026", "Writing\u2026"),
                "complete": (
                    "Preparing the sounding display\u2026", "Preparing\u2026"),
                "rendering": (
                    "Rendering the sounding window\u2026", "Rendering\u2026"),
            }
        ...
        action_button = self._model_fetch_btn
        if getattr(self, "_model_busy_kind", None) == "cache":
            action_button = self._model_cache_btn
        action_button.setText(button)
    ...
    def _poll_model_fetch_progress(self) -> None:
        ...
        if self._model_progress_stage != "downloading":
            return
        worker = self._model_worker or getattr(
            self, "_model_cache_worker", None)
        if worker is None:
            return
        downloaded = 0
        ...
        if total > 0:
            ...
            if getattr(self, "_model_busy_kind", None) == "cache":
                action_button = self._model_cache_btn
            action_button.setText(f"Downloading\u2026 {percent}%")
    ...
    def _on_model_fetch_ok(self, npz_path, label, run_time, fxx,
                           prof_col=None, stn_id=None) -> None:
        worker = self.sender()
        ...
        QApplication.processEvents()
        try:
            # New workers can hand over a decoded/warmed collection. Retain the
            # decode fallback for the historical four-value signal.
            if prof_col is None:
                prof_col, stn_id = _render().decode(npz_path)
            title = (
                f"{APP_NAME} \u2014 {label} "
            ...
            if self._model_viewer_is_open():
                ...
                menu_name = self._replace_model_profile(win, prof_col)
                ...
                menu_name = win.createMenuName(prof_col)
            self._track_model_viewer(win, menu_name)
            _retain_model_data_until_close(
    ...
    def _cancel_model_fetch(self) -> None:
        ...
        if self._model_cache_worker is worker:
            self.statusBar().showMessage("Cancelling model-hour cache\u2026")
    ...
    def _start_model_prefetch(
        ...
        if not self._model_prefetch_enabled() or self._model_is_busy():
            return
    ...
    def _clear_model_cache(self) -> None:
        if self._model_is_busy():
            QMessageBox.information(
                self, APP_NAME,
                "Wait for the active model operation to finish or cancel it first.",
            )
            return
        self._cancel_model_prefetch(wait=True)
        self._model_hour_cache.clear()
    ...
    def _open_file(self, path: str) -> None:
        ...
        if self._file_worker is not None:
            QMessageBox.information(
                self, APP_NAME, "A local sounding is already loading.")
            return

        # Keep decoding and expensive derived calculations off Qt's UI thread.
        # The focused worker module owns this class; importing lazily preserves
        # the picker's fast startup path.
        from sharpmod.gui_workers import _FilePrepareWorker
        ...
        worker = _FilePrepareWorker(path, parent=self)
        self._file_worker = worker
        worker.finished_ok.connect(self._on_file_prepare_ok)
        worker.failed.connect(self._on_file_prepare_failed)
        worker.finished.connect(self._on_file_worker_finished)
        worker.start()
