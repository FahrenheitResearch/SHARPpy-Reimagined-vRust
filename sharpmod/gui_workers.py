...
class _FetchWorker(QThread):
    ...
    Emits the saved path plus a decoded, display-warmed profile collection so
    MetPy imports and ECAPE never block Qt while the window is being composed.
    """
    ...
    def run(self):  # noqa: D401 - QThread entry point
        ...
        self.finished_ok.emit(
            npz_path, meta, self._when, prof_col, stn_id)
...
def _prepare_display_profile(prof_col):
    """Resolve the expensive displayed lazy fields on the worker thread."""
...
class _ModelFetchWorker(QThread):
    ...
    def _extract_python(self, model_extract, cfg, run_dt):
        """Use upstream's point/subregion/cache stack for one exact run."""
        if self._model_hour_cache is None:
            return model_extract.extract(
                self._model,
                self._lat,
                self._lon,
                run_time=run_dt,
    ...
    def _native_progress(self, message, _percent=0):
        ...
        if "cached" in text or "cache" in text:
            ...
            stage = "writing"
        elif "try" in text or "provider" in text:
            stage = "downloading"
        else:
            stage = "locating"
    ...
    def run(self):  # noqa: D401 - QThread entry point
        ...
        try:
            ...
            mode = rusty_weather.backend_mode()
            native_available = rusty_weather.is_available(cfg.key)
            native_cached = native_available and rusty_weather.hour_is_cached(
                cfg.key, run_dt, self._fxx)

            if native_cached:
                # The full model hour already exists locally. Rust point export
                # is typically tens of milliseconds and must win every time.
                path, resolved_run = rusty_weather.extract_cached(
                    cfg.key, self._lat, self._lon, run_dt, self._fxx,
                    self._out_path, label=cfg.label, loc=self._loc,
                    progress=self._native_progress,
                    cancelled=self.cancellation_requested)
            elif mode == "rust":
                # Explicit native mode accepts the larger one-time full-hour
                # transfer in exchange for instant arbitrary map browsing.
                path, resolved_run = rusty_weather.extract(
                    cfg.key, self._lat, self._lon, run_dt, self._fxx,
                    self._out_path, label=cfg.label, loc=self._loc,
                    progress=self._native_progress,
                    cancelled=self.cancellation_requested)
                ...
                try:
                    # Cold interactive requests use the smallest transport:
                    # HRRR Zarr point, NOMADS subregion/indexed ranges, then
                    # Herbie. A cold Rust full-hour ingest is much larger.
                    path = self._extract_python(model_extract, cfg, run_dt)
                    resolved_run = run_dt
                except model_extract.DownloadCancelled:
                    raise
                except Exception as python_exc:
                    if not native_available or mode != "auto":
                        raise
                    _LOGGER.warning(
                        "model_fetch.python_failed_native_fallback model=%s "
                        "run=%s fxx=%03d reason=%s",
                        cfg.key, run_dt, self._fxx, python_exc)
                    try:
                        path, resolved_run = rusty_weather.extract(
                            cfg.key, self._lat, self._lon, run_dt, self._fxx,
                            self._out_path, label=cfg.label, loc=self._loc,
                        ...
                        raise model_extract.RetrievalError(
                            "optimized point providers failed (%s); native "
                            "full-hour fallback also failed (%s)"
                            % (python_exc, native_exc)
                        ) from native_exc
        ...
        _LOGGER.info(
            ...
            self._model, resolved_run, self._fxx, path)
        self.finished_ok.emit(
...
class _NativeHourCacheWorker(QThread):
    """Build one selected Rust ``.rws`` hour without blocking Qt.

    The result does not open or replace a sounding.  It simply makes later
    clicks for the exact model/run/forecast hour eligible for the native
    cached-point fast path.
    """

    # model key, resolved UTC run, forecast hour, durable .rws path
    finished_ok = Signal(str, object, int, str)
    failed = Signal(str)
    cancelled = Signal()
    # Uses the same stage/byte contract as _ModelFetchWorker so the picker can
    # reuse its progress rail and elapsed/download-size polling.
    progress = Signal(str, int)

    def __init__(self, model: str, run_time: datetime, fxx: int, parent=None):
        super().__init__(parent)
        ...
        self._cancel_requested = False
        from sharpmod.tools import rusty_weather
        self._download_dir = os.fspath(
            rusty_weather.default_cache_root() / "downloads")
    ...
    def _native_progress(self, message, _percent=0):
        ...
        if "ready" in text:
            stage = "complete"
        elif "cached" in text or "cache" in text:
            stage = "cached"
        elif "try" in text or "provider" in text:
            stage = "downloading"
        else:
            stage = "locating"
        self.progress.emit(stage, 0)
    ...
    def run(self):  # noqa: D401 - QThread entry point
        try:
            ...
            cfg = model_extract.get_config(self._model)
            run_dt = model_extract._run_datetime(self._run_time, cfg)
            if not rusty_weather.is_available(cfg.key):
                if cfg.key not in rusty_weather.SUPPORTED_MODELS:
                    detail = f"{cfg.label} is not supported by Rusty Weather"
                elif rusty_weather.backend_mode() == "python":
                    detail = (
                        "Rust acquisition is disabled by "
                        "SHARPMOD_MODEL_BACKEND=python"
                    )
                else:
                    detail = "the native cache helpers are not installed"
                raise model_extract.RetrievalError(detail)
            hour_path = rusty_weather.cache_hour(
                cfg.key,
                run_dt,
                self._fxx,
                progress=self._native_progress,
            ...
            self.cancelled.emit()
            return
        except Exception as exc:  # noqa: BLE001 - surface native/provider error
            _LOGGER.exception(
                "native_hour_cache.worker_failed model=%s run=%s fxx=%03d",
                self._model, self._run_time, self._fxx)
            self.failed.emit(f"Could not cache this model hour: {exc}")
            return

        _LOGGER.info(
            "native_hour_cache.worker_ok model=%s run=%s fxx=%03d path=%s",
            cfg.key, run_dt, self._fxx, hour_path)
        self.progress.emit("complete", 0)
        self.finished_ok.emit(
            cfg.key, run_dt, self._fxx, os.fspath(hour_path))


class _FilePrepareWorker(QThread):
    """Decode and warm one local sounding without blocking Qt's event loop."""
    ...
    def run(self):
        try:
            from sharpmod.io.decoder import load_npz
            prof_col, stn_id = load_npz(self._path)
            _prepare_display_profile(prof_col)
        except Exception as exc:  # noqa: BLE001 - worker surfaces the error
