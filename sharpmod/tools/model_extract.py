...
def extract_with_cycle_fallback(
    ...
    # This compatibility API still permits explicit earlier-cycle fallback.
    # The v0.3 GUI instead exposes "Use available cycle" and never silently
    # changes the selection. Within each candidate, prefer an existing Rust
    # full-hour cache, then the small optimized Python transports; a cold Rust
    # ingest is a late fallback because it transfers an entire model hour.
    from sharpmod.tools import rusty_weather
    mode = rusty_weather.backend_mode()
    native_available = rusty_weather.is_available(config.key)
    if mode == "rust" \
    ...
    if native_available and mode == "auto":
        for candidate in candidates:
            if not rusty_weather.hour_is_cached(config.key, candidate, fxx):
                continue
            try:
                return rusty_weather.extract_cached(
    ...
    if native_available and mode in {"auto", "rust"}:
        for candidate in candidates:
            try:
