...
- Added a pinned, stable-ABI `sharpmod_native` extension that runs one bulk,
  GIL-free sounding analysis through the `sharppyrs` display layer and its
  `sharprs` core. It supplies normalized arrays, parcels and traces,
  thermodynamics, kinematics, severe composites, fire/winter values, watch
  inputs, and all 84 current `sharppyrs` derived fields.
- Routed interactive user-parcel lifts through `sharprs` and authoritative
  native ECAPE/NCAPE through the in-process `ecape-rs` implementation.
- Added a versioned `sharpmod.native-analysis.v1` result contract with explicit
  calculation provenance and exact `sharppyrs`, `sharprs`, and `ecape-rs`
  source revisions exposed by the frozen-runtime probe.
- Retained explicit Python compatibility boundaries for detailed fire-PBL
  fields, precipitation-source/layer-energy analysis, SARS analog databases,
  and station PWV climatology. A missing, disabled, rejected, or failed native
  analysis falls back to the complete Python `ConvectiveProfile`.
- Added **Cache This Hour for Fast Map Browsing**. It builds a selected HRRR,
  GFS, or RRFS-A `.rws` hour in an independent background worker with progress,
  cancellation, bounded retention, and source-GRIB cleanup.
- Configured release targets for a Windows x64 one-folder ZIP and single EXE,
  a Linux x64 one-folder tarball and single executable, and zipped macOS
  application bundles for Apple Silicon and Intel. These are build targets;
  this unreleased changelog does not claim that v0.3.2 artifacts are published.

### Performance and UX

- Forecast retrieval now checks an exact Rust `.rws` cache hit first. A cold
  request uses the HRRR F000 Zarr point path where applicable, then optimized
  NOMADS geographic subsets or indexed ranges, then standard Herbie. Cold Rust
  full-hour ingestion is reserved for a late automatic fallback or an explicit
  `SHARPMOD_MODEL_BACKEND=rust` request.
- `SHARPMOD_MODEL_BACKEND=python` now cleanly disables Rust acquisition while
  retaining the optimized Python point/subregion paths and Herbie fallback.
- Sounding decoding and display-derived calculations run off the Qt event loop.
  Forecast fetches expose stage/byte/elapsed-time progress and cancellation,
  and the prepared viewer is shown without an empty initial paint.
- After the first forecast-model sounding opens, a single click on another map
  point replaces the forecast profile in that same interactive viewer instead
  of opening a new window.
- On the bundled 39-level HRRR development sample, release-mode native analysis
  measured 1.4–1.9 ms per bulk call, interactive parcel lifting 0.72–0.85 ms,
  and the complete compatible profile 39–48 ms including the intentional
  Python-only features. These are development measurements, not guarantees.

### Fixed

- Synced the upstream v0.3.1 fixes: scalar coordinates from zero-area ERA5 CDS
  responses are accepted, NCAPE work is limited to the equilibrium level to
  avoid undefined upper-stratospheric saturation calculations, and Windows
  PyInstaller builds embed and verify the local `sharpmod` package.
- Treats absent objects in the sparse HRRR Zarr archive as fill values instead
  of aborting an otherwise valid point request.
- Application, package, window, and rendered-image branding now share the
  vRust v0.3.2 version source.
